...
class SSLSocket {
	...
	static var SSL_new = neko.Lib.load("opensslndll", "_SSL_new", 1);
	static var BIO_new_socket = neko.Lib.load("opensslndll", "_BIO_new_socket", 2);
	static var SSL_set_bio = neko.Lib.load("opensslndll", "_SSL_set_bio", 3);
	static var BIO_NOCLOSE = neko.Lib.load("opensslndll", "_BIO_NOCLOSE", 0);
	static var SSL_connect = neko.Lib.load("opensslndll", "_SSL_connect", 1);
	static var SSL_set_fd = neko.Lib.load ("opensslndll", "_SSL_set_fd", 2);
	static var SSL_CTX_set_verify_depth = neko.Lib.load("opensslndll", 
													"_SSL_CTX_set_verify_depth", 2);
	static var BIO_new = neko.Lib.load("opensslndll", "_BIO_new", 1);
	static var BIO_set_fd = neko.Lib.load("opensslndll", "_BIO_set_fd", 3);
	static var BIO_s_socket = neko.Lib.load("opensslndll", "_BIO_s_socket", 0);
	...
	private function initializeOpenSSL(){
		...
		var rsclvl : Int = SSL_CTX_load_verify_locations(
				ctx, neko.Lib.haxeToNeko("E:\\openssl-0.9.8c\\certs"));
		// unimplemented
	...
	public function new( ?s ) {
		...
		input = new neko.io.SocketInput(__s);
		output = new neko.io.SocketOutput(__s);
	}
	...
	public function write( content : String ) {
		socket_write(__s, neko.Lib.haxeToNeko(content));
	}
	...
	public function connect(host : Host, port : Int) {
		try {
			...
			//var sbio=BIO_new(BIO_s_socket());
			//BIO_set_fd(sbio, __s, BIO_NOCLOSE());
